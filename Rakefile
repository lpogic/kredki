...
when /linux/
  ...
  desc "Build pastele & update project binaries"
  task :build => "pastele:build"
  task :build => "kredki/stuff/dylib"
  task :build => "thorvg:import"
  task :build => "sdl:import"
  task :build => "pastele:import"
  
  directory "kredki/stuff/dylib"
  ...
  namespace :thorvg do
    ...
    task :import do
      check_vars :$thorvg
      target = "kredki/stuff/dylib/libthorvg-1.1.dylib"
      source = "#$thorvg/builddir/src/libthorvg-1.1.dylib"
      cp source, target
    end
  end
  ...
  namespace :sdl do
    ...
    task :import do
      check_vars :$sdl
      target = "kredki/stuff/dylib/libSDL3.0.dylib"
      source = "#$sdl/builddir/libSDL3.0.dylib"
      source = "#$sdl/lib/libSDL3.0.dylib" unless File.exist? source
      cp source, target
    end
  end
  ...
  namespace :pastele do
    ...
    make = task :make do
      check_vars :$sdl, :$thorvg
      check_vars :$make, file: false
      sdl_dylib = "#$sdl/builddir/libSDL3.0.dylib"
      sdl_dylib = "#$sdl/lib/libSDL3.0.dylib" unless File.exist? sdl_dylib
      File.write "pastele/CMakeLists.txt", <<~xx
        cmake_minimum_required(VERSION 3.10)
        ...
        target_link_libraries(pastele PRIVATE
          "#$thorvg/builddir/src/libthorvg-1.1.dylib"
          "#{sdl_dylib}"
        )
      xx
    ...
    task :import do
      target = "kredki/stuff/dylib/libpastele.dylib"
      source = "pastele/cmaked/libpastele.dylib"
      cp source, target
    end
  end
else
