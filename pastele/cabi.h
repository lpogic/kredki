...
// CABI int tvg_swcanvas_set_mempool(SwCanvas* canvas, int policy);
// CABI int tvg_canvas_delete(Canvas* canvas);
...
// CABI int tvg_shape_get_stroke_color(const Shape* self, uint8_t* r, uint8_t* g, uint8_t* b, uint8_t* a);
...
// CABI int tvg_shape_get_fill_rule(const Shape* self, Tvg_Fill_Rule* rule);
...
// TVG_DEPRECATED CABI int tvg_scene_reserve(Tvg_Paint* scene, uint32_t size);
...
// CABI int tvg_lottie_animation_get_marker(Animation* self, uint32_t idx, const char** name);
