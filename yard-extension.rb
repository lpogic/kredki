class ParamHandler < YARD::Handlers::Ruby::Base
  ...
  process do
    ...
    definition.docstring =  <<~xx
      @note This is the primary setter of +#{name_base}+ _parameter_. +##{name_base}=+ and +##{name_base}+ methods are also defined. Check *Flags and Parameters* for more info.
      #{statement.docstring}
    xx
...
class FlagHandler < YARD::Handlers::Ruby::Base
  ...
  process do
    ...
    definition.docstring =  <<~xx
      @note This is the primary setter of +#{name_base}+ _flag_. +##{name_base}=+, +##{name_base}+ and +##{name_base}?+ methods are also defined. Check *Flags and Parameters* for more info.
      @param #{arg_name} [true, false, :not] Set if +true+, unset if +false+, negate if +:not+.
      #{statement.docstring}
    xx
    # definition.source = "flag " + statement.source
    parse_block(definition, namespace: namespace)
  end
end